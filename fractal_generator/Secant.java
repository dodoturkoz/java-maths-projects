...
public class Secant {
     ...
     * program may appear to freeze. normal = 20
     */
    public static final int MAXITER = 20;

    /**
     * The TOL that should be used throughout this project. Note that
     * you should reference this variable and _not_ explicity write out
     * 1.0e-10 in your solution code. Other classes can access this TOL
     * by using Secant.TOL.
     */
     ...
     * order to find a root within TOL TOL.
     *
    ...
    public void iterate(Complex z0, Complex z1) {
        ...
        // initialise numit so when called repeateadly it doesn't keep growing
        numIterations = 0;
        while (numIterations < MAXITER) {
            ...
            // f(z0)
            Complex evalz0 = f.evaluate(z0);
            // f(z1)
            Complex evalz1 = f.evaluate(z1);
            // z1-z0
            Complex z1minusz0 = z1.subtract(z0);
            // f(z1)-f(z0)
            Complex evaldiff = evalz1.subtract(evalz0);
            Complex z2 = z1.subtract(evalz1.multiply(z1minusz0.divide(evaldiff)));
            // update z0 and z1
            z0 = z1;
            z1 = z2;
            numIterations++;

            if ((z1.subtract(z0)).abs() < TOL && (f.evaluate(z1)).abs() < TOL) {
                err = Error.OK;
                root = z1;
                return;

            }
            // eger |f(z1)-f(z0)|=0 set error to ZERO
            if (evaldiff.abs() < TOL) {
                err = Error.ZERO;
                return;
            }
            // if everything goes fine set error to OK and set root to z1

        }
        // if numiter exceeds MAXITER set error to DNF
        err = Error.DNF;
    }
    ...
    public static void main(String[] args) {
        ...
        // // Basic tester: find a root of f(z) = z^3-30.
        // Complex[] coeffd = new Complex[] { new Complex(-30.0, 0.0), new Complex(),
        // new Complex(),
        // new Complex(1.0, 0.0) };
        // final Polynomial q = new Polynomial(coeffd);
        // Secant t = new Secant(q);

        // t.iterate(new Complex(20.0, 0.0), new Complex(37.0, 0.0));
        // System.out.println(t.getNumIterations());
